      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        if (isAlpha(ch)) {
          ...
          if (state === 2 || state === 4 || state === 5) {
            ...
            if ((!isOperator(ch) || ch === ".") && !isDigit(ch)) error = true;
          }
          ...
          if (canAddPlusOp(ch)) {
            ...
            if (ch === "=") token += ch;
              ...
              // could repeast
              if (canRepeat(arr[i - 1]) && ch === arr[i - 1]) token += ch;
              else --i;
            }
          }
          //判断是否为界符
